...
.nsb-editor .nsb-compact-control{display:grid!important;grid-template-columns:52px minmax(0,1fr) 56px;gap:7px;align-items:center}.nsb-editor .nsb-compact-control label{min-width:0!important}.nsb-editor .nsb-compact-control input[type="range"]{width:100%!important;min-width:0}.nsb-editor .nsb-compact-control .nsb-num{width:56px!important;padding:6px 7px}
...
.nsb-editor-tabs{display:flex;gap:4px;margin:12px 0 9px;border-bottom:2px solid rgba(213,139,174,.35)}.nsb-editor-tab{border:0;border-radius:9px 9px 0 0;padding:8px 13px;background:transparent;color:#8c7180;font-size:13px;cursor:pointer}.nsb-editor-tab.active{background:rgba(255,255,255,.68);color:#8f3f67;font-weight:700;box-shadow:0 -1px 7px rgba(139,65,101,.08)}.nsb-editor-page[hidden]{display:none!important}
...
export default {
  ...
  setup(ctx) {
      ...
      };
      {
        ...
        function render() {
          ...
          if (!selected) {
            ...
            for (const [key, label] of [["slice", "切割线"], ["padding", "内边距"], ["font", "字体"]]) {
            ...
            main.append(editorTabs, editorPages.slice, editorPages.padding, editorPages.font);
            syncEditorTabs();
            ...
            visualRow.className = "nsb-row nsb-compact-control";
            ...
            visualRow.append(scaleLabel, scaleRange, scaleInput);
            main.insertBefore(visualRow, editorTabs);
    ...
    ctx.ui.slot("settings.section", (el) => {
      ...
      const tools = document.createElement("div");
      tools.className = "nsb-plugin-tools";
      const importInput = document.createElement("input");
      importInput.type = "file";
      importInput.accept = "application/json,.json";
      importInput.style.display = "none";
      const makeTool = (label, svg, onClick) => {
        const button = document.createElement("button");
        button.type = "button";
        button.className = "nsb-plugin-tool";
        button.title = label;
        button.setAttribute("aria-label", label);
      ...
      const importButton = makeTool("导入配置", '<svg viewBox="0 0 24 24"><path d="M12 16V4M8 8l4-4 4 4M4 15v4a1 1 0 0 0 1 1h14a1 1 0 0 0 1-1v-4"/></svg>', () => importInput.click());
      const exportButton = makeTool("导出配置", '<svg viewBox="0 0 24 24"><path d="M12 4v12M8 12l4 4 4-4M4 15v4a1 1 0 0 0 1 1h14a1 1 0 0 0 1-1v-4"/></svg>', () => {
        const blob = new Blob([JSON.stringify(state, null, 2)], { type: "application/json" });
        const url = URL.createObjectURL(blob);
        const anchor = document.createElement("a");
        anchor.href = url;
        anchor.download = "float-bubble-skins.json";
        anchor.click();
        setTimeout(() => URL.revokeObjectURL(url), 1000);
      });
      tools.append(importButton, exportButton, importInput);

    ...
    ctx.hooks.transform(
      ...
      function (payload) {
        ...
        var block =
          ...
          "\n如果剧情发生时间跳跃，标记写跳跃后的时间。没有跳跃则写当前时间。" +
          "\n不要在正文里提及时间标记，也不要用其他格式输出时间数字串。" +
          "\n如果本次输出包含 <summary> 摘要段落，摘要开头必须先写明本轮剧情发生的虚拟时间（格式如「" +
          formatDisplay(getVirtualNow(state)) +
          "」），再写事件内容。系统记录的真实日期与虚拟时间无关，一律以你写的虚拟时间为准。" +
          buildTimelineFromMessages();
...
function exitFollowMode() {
      ctx.system.storage.set("followYear", "false");
      ctx.system.storage.set("followDate", "false");
      ctx.system.storage.set("followTime", "false");
    }
    function askExitFollow(ts, source, onConfirm) {
      ctx.ui.openModal(function (el, modal) {
        el.style.cssText =
          "padding:24px;max-width:320px;text-align:center;background:#fff;border-radius:14px;";
        el.innerHTML =
          '<div style="font-size:16px;color:#1a1a1a;font-weight:600;margin-bottom:14px">剧情要求时间跳转</div>' +
          '<div style="font-size:13px;color:#999;margin-bottom:8px">来源：' + source + '</div>' +
          '<div style="font-size:13px;color:#666;margin-bottom:4px">目标时间：</div>' +
          '<div style="font-size:20px;color:#1a1a1a;font-weight:600;margin-bottom:14px">' + formatDisplay(ts) + '</div>' +
          '<div style="font-size:12px;color:#e67e22;line-height:1.6;margin-bottom:20px">当前处于「跟随系统时间」模式，时间由真实时钟决定，无法跳转。<br>是否退出跟随模式并跳转？</div>' +
          '<div style="display:flex;gap:10px;justify-content:center">' +
          '<button id="vc-fy" style="padding:10px 20px;border:1.5px solid #e8e0ce;border-radius:10px;background:#FDF8F0;color:#555;font-size:14px;font-weight:500;cursor:pointer">退出并跳转</button>' +
          '<button id="vc-fn" style="padding:10px 20px;border:1.5px solid #f0e8d8;border-radius:10px;background:#FFFCF5;color:#555;font-size:14px;font-weight:500;cursor:pointer">保持跟随</button>' +
          '</div>';
        el.querySelector("#vc-fy").onclick = function () {
          exitFollowMode();
          modal.close();
          onConfirm();
        };
        el.querySelector("#vc-fn").onclick = function () {
          modal.close();
        };
      });
    }
    ...
    function handleTimeJump(ts, source) {
      if (isFollowMode()) {
        var jm = ctx.system.settings.get("jumpMode") || "confirm";
        if (jm === "manual") return;
        askExitFollow(ts, source, function () {
          ...
          ctx.ui.toast("已退出跟随模式，跳转至 " + formatDisplay(ts));
        });
        return;
      }
